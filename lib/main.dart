...
//
//
//       initialRoute: isLoggedIn ? AppPages.main : AppPages.login,
    ...
    runApp(
      MultiProvider(
        providers: [
          ...
          ChangeNotifierProvider(
              create: (context) => OrderPaidProvider(context.dataProvider)),
          ChangeNotifierProvider(
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetMaterialApp(
      ...
      // ✅ به‌جای initialRoute از home استفاده کن تا Navigator خودش روتِ '/' را
      // با یک صفحه‌ی واحد بسازد و «صفحه‌ی زیرینِ ناخواسته» ایجاد نشود.
      // این کار جلوی سناریوهایی مثل:
      //  - دو بار لود شدن Login
      //  - رفتن به داشبورد بعد از Logout با Back
      // را می‌گیرد.
      home: isLoggedIn ? MainScreen() : LoginScreen(),

      // اگر جایی از پروژه ناوبری Named داشتی، این‌ها را نگه می‌داریم.
      getPages: AppPages.routes,

      // بهتره ناشناخته‌ها هم برن لاگین، نه داشبورد.
