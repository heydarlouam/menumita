...
//     // 1) اول state رو پاک کن + realtime رو ببند
//     await context.dataProvider.logoutCleanup();
//
//     // 2) بعد shared prefs رو پاک کن
//     await PrefsService.clearPrefix('user');
//
//     // 3) بستن Drawer اگر بازه
//     if (Scaffold.maybeOf(context)?.isDrawerOpen == true) {
//       Navigator.of(context).pop();
//     }
//
//     // 4) برو لاگین (همه route ها پاک)
...
class SideMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Map<String, dynamic>?>(
      ...
      builder: (context, snap) {
        final menuType =
        (snap.data?['menu_type'] ?? '').toString().trim().toLowerCase();
        final hideOrdersAndCoupons = menuType == 'menu_one';
  ...
  Future<void> _logout(BuildContext context) async {
    ...
    // 2) پاک‌سازی SharedPreferences + پاک شدن کش‌های UserSaveHelper
    await UserSaveHelper.logout();
