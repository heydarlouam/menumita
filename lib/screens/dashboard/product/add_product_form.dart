...
class _ProductSubmitFormState extends State<ProductSubmitForm> {
  ...
  double _dialogWidth(double screenW) {
    if (_isMobile(screenW)) return (screenW * 0.96).clamp(320.0, 560.0);
    if (_isTablet(screenW)) return (screenW * 0.85).clamp(560.0, 900.0);
    return (screenW * 0.70).clamp(900.0, 1200.0);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isTablet = _isTablet(screenW);

    final formW = _dialogWidth(screenW);
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // ✅ تصاویر: در موبایل/تبلت Wrap می‌شوند
              LayoutBuilder(
                builder: (_, cons) {
                  final availableW = cons.maxWidth;

                  // حداقل عرض هر کارت (بهتر برای Wrap)
                  double cardW;
                  if (availableW < 420) {
                    cardW = (availableW - defaultPadding) / 2; // 2 ستونه
                  } else if (availableW < 900) {
                    cardW = (availableW - defaultPadding * 2) / 3; // 3 ستونه
                  } else {
                    cardW = (availableW - defaultPadding * 4) / 5; // 5 ستونه
                  }
                  ...
                  return Wrap(
                    ...
                    children: [
                      SizedBox(width: cardW, child: _buildImageCard(context, 1, 'تصویر اصلی')),
                      SizedBox(width: cardW, child: _buildImageCard(context, 2, 'تصویر دوم')),
                      SizedBox(width: cardW, child: _buildImageCard(context, 3, 'تصویر سوم')),
                      SizedBox(width: cardW, child: _buildImageCard(context, 4, 'تصویر چهارم')),
                      SizedBox(width: cardW, child: _buildImageCard(context, 5, 'تصویر پنجم')),
                    ],
                  );
                },
              ),
              ...
              CustomTextField(
                ...
                onSave: (val) {},
                validator: (value) {
                  if (value == null || value.isEmpty) return 'لطفاً نام را وارد کنید';
                  return null;
                },
              ),
              ...
              // ✅ نوع ویژگی + انتخاب چندتایی ویژگی‌ها
              if (isMobile) ...[
                _buildVariantTypeDropdown(context),
                const SizedBox(height: defaultPadding),
                _buildVariantsMultiSelect(context),
              ] else ...[
                Row(
                  children: [
                    Expanded(child: _buildVariantTypeDropdown(context)),
                    ...
                    Expanded(
                      flex: isTablet ? 2 : 2,
                      child: _buildVariantsMultiSelect(context),
                    ),
                  ],
                ),
              ],
                    ...
                    return SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: AnimatedSwitcher(
                          ...
                          transitionBuilder: (child, anim) => FadeTransition(opacity: anim, child: child),
                          child: isBusy
                              ? const SubmissionSpinner(key: ValueKey('product_loading'))
                              : const Text(
                            'ثبت',
                            key: ValueKey('product_submit_text'),
                            style: TextStyle(fontFamily: FONTS_STYLE_FAMILY),
  ...
  // dropdown ها (همون قبلی، بدون تغییر)
  Widget _buildCategoryDropdown(BuildContext context) {
    return Consumer<DashBoardProvider>(
      builder: (context, dashProvider, child) {
        return Selector<DataProvider, List<Category>>(
          ...
          builder: (_, categories, __) {
            ...
            return CustomDropdown(
              ...
              validator: (value) {
                if (value == null) return 'لطفاً یک دسته‌بندی انتخاب کنید';
                return null;
              },
            );
          },
  ...
  Widget _buildSubCategoryDropdown(BuildContext context) {
    return Consumer<DashBoardProvider>(
      builder: (context, dashProvider, child) {
        final sorted = List<SubCategory>.from(dashProvider.subCategoriesByCategory)
          ..sort((a, b) => (a.name ?? '').compareTo(b.name ?? ''));
        ...
        return CustomDropdown(
          ...
          validator: (value) {
            if (value == null) return 'لطفاً یک زیر‌دسته انتخاب کنید';
            return null;
          },
        );
      },
  ...
  Widget _buildBrandDropdown(BuildContext context) {
    return Consumer<DashBoardProvider>(
      builder: (context, dashProvider, child) {
        final sorted = List<Brand>.from(dashProvider.brandsBySubCategory)
          ..sort((a, b) => (a.name ?? '').compareTo(b.name ?? ''));
        ...
        return CustomDropdown(
          ...
          validator: (value) {
            if (value == null) return 'لطفاً برند را انتخاب کنید';
            return null;
          },
        );
      },
    ...
    return Consumer<DashBoardProvider>(
      builder: (context, dashProvider, child) {
        return Selector<DataProvider, List<VariantType>>(
          selector: (_, dp) => dp.variantTypes,
          builder: (_, types, __) {
            final items = List<VariantType>.from(types)
              ..sort((a, b) => (a.name ?? '').compareTo(b.name ?? ''));
            return CustomDropdown(
              key: ValueKey(dashProvider.selectedVariantType?.sId),
              initialValue: dashProvider.selectedVariantType,
