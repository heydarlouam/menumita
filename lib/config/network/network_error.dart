...
class NetworkError {
  ...
  /// پیام فنی‌تر برای لاگ و دیباگ (برای سازگاری nullable نگه داشته شده)
  ...
  /// برای سازگاری با کدهایی که از `error.message` استفاده می‌کنند
...
// نکته: response در بعضی نسخه‌ها String? است، پس آن را Object? می‌گیریم
    final Object? resp = exception.response;

    if (resp == null) {
      details = null;
    } else if (resp is Map) {
      details = resp.map((k, v) => MapEntry(k.toString(), v));
    } else if (resp is String) {
      // بعضی وقت‌ها response یک JSON string است
      try {
        final decoded = jsonDecode(resp);
        if (decoded is Map) {
          details = decoded.map((k, v) => MapEntry(k.toString(), v));
        } else {
          details = <String, dynamic>{'response': resp};
        }
      } catch (_) {
        details = <String, dynamic>{'response': resp};
      }
